...
uint32_t dfu_init_pkt_handle(dfu_update_packet_t * p_packet)
{
    ...
    switch (m_dfu_state)
    {
        ...
        case DFU_STATE_RX_INIT_PKT:
            ...
            {
                m_init_packet[m_init_packet_length++] =
                    p_packet->params.data_packet.p_data_packet[i];
            }
            break;
...
static uint32_t dfu_compare_block(uint32_t * ptr1, uint32_t * ptr2, uint32_t len)
{
    ...
    sd_mbr_cmd.command             = SD_MBR_COMMAND_VERIFY;
    sd_mbr_cmd.params.verify.ptr1 = ptr1;
    sd_mbr_cmd.params.verify.ptr2 = ptr2;
    sd_mbr_cmd.params.verify.len  = len / sizeof(uint32_t);
...
static uint32_t dfu_copy_sd(uint32_t * src, uint32_t * dst, uint32_t len)
{
    ...
    sd_mbr_cmd.command            = SD_MBR_COMMAND_COPY;
    sd_mbr_cmd.params.copy.src = src;
    sd_mbr_cmd.params.copy.dst = dst;
    sd_mbr_cmd.params.copy.len = len / sizeof(uint32_t);
...
uint32_t dfu_bl_image_validate(void)
{
    ...
    if (bootloader_settings.bl_image_size != 0)
    {
        ...
        sd_mbr_cmd.command             = SD_MBR_COMMAND_VERIFY;
        sd_mbr_cmd.params.verify.ptr1 = (uint32_t *)BOOTLOADER_REGION_START;
        sd_mbr_cmd.params.verify.ptr2 = (uint32_t *)(bl_image_start);
        sd_mbr_cmd.params.verify.len  = bootloader_settings.bl_image_size / sizeof(uint32_t);
...
uint32_t dfu_sd_image_validate(void)
{
    ...
    sd_mbr_cmd.command             = SD_MBR_COMMAND_VERIFY;
    sd_mbr_cmd.params.verify.ptr1 = (uint32_t *)SOFTDEVICE_REGION_START;
    sd_mbr_cmd.params.verify.ptr2 = (uint32_t *)bootloader_settings.sd_image_start;
    sd_mbr_cmd.params.verify.len  = bootloader_settings.sd_image_size / sizeof(uint32_t);
